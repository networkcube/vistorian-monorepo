...
export function findTemplate(nodes, template, config) {
...
export function findClusters(nodes, config) {
    if (nodes.length == 0)
        return [];
    const g = nodes[0].g;
    const links = nodes[0].g.links().toArray();
...
*/
export function findFullEgoNetwork(nodes, config) {
    const motifs = [];
    let ns;
    let ls;
    let finalLinks;
