...
import { offsetInMilliseconds, addDate, startOf, formatAsDDMMYYYY, } from "../data/dates";
export class Timeline {
    constructor(webgl, network, x, y, width, height) {
        ...
        this.dayOfWeek = timeFormat("%w"); // "Sunday-based weekday as a decimal number [0,6]."
        this.x = x;
        this.y = y;
        this.WIDTH = width;
        this.HEIGHT = height;
    ...
    visualize() {
        ...
        const numTimes = Math.ceil(Math.abs((end.getTime() - start.getTime()) /
            offsetInMilliseconds[this.granules[this.minGran]])); // WITHOUT 's'
        ...
        for (let i = 0; i < this.timeObjects.length; i++) {
            ...
            for (let gran = this.maxGran; gran >= this.minGran && !granularitySet; gran--) {
                ...
                else if (utils.formatAtGranularity(to1, gran) !=
    ...
    updateWithIds(minTimeId, maxTimeId) {
        ...
        for (let g = this.minGran; g < this.maxGran && this.tick_minGran_visible == undefined; g++) {
            ...
            const ticks = (granularityIndex) => (t2.getTime() - t1.getTime()) /
            ...
            if (g <= 7) {
                ...
                if (g == 8)
                    requiredTicks = ticks(7) / 10;
                if (g == 9)
                    requiredTicks = ticks(7) / 100;
                if (g == 10)
                    requiredTicks = ticks(7) / 1000;
            }
