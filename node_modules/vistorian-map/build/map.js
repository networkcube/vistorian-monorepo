...
const locations = dgraph.locations().toArray();
for (let i = 0; i < locations.length; i++) {
    locations[i]["npos"] = [];
}
let time_start = dgraph.time(0); // BEFORE queries.Time
let time_end = dgraph.times().last(); // BEFORE queries.Time
// world map to show node positions
const mapCanvas = d3.select("#visDiv").node();
...
// VISUAL ELEMENTS
const nodePositionObjects = [];
const nodePositionObjectsLookupTable = [];
let geoMultiLinks = [];
class GeoMultiLink {
    constructor() {
        this.links = [];
...
export class NodePositionObject {
    constructor() {
        ...
        this.xOrig = 0; // INIT ??
        this.yOrig = 0; // INIT ??
        this.geoPos = new google.maps.LatLng(0, 0); // INIT ??
        this.displaced = false; // INIT ??
        this.displacementVector = []; // INIT ??
...
let visualLinks;
let vNodeLabels;
let vNodeLabelBackgrounds;
let geoProjection;
let layer;
let locationsPanelDiv;
const locationDisplayTimeoutHandle = -1;
let prevIntersectedLink;
let prevIntersectedNode;
let intersectedLink;
let intersectedNode;
const F = 1000;
...
function init() {
    ...
    map.addListener("zoom_changed", function () {
        ...
        const p = (_a = weirdDiv === null || weirdDiv === void 0 ? void 0 : weirdDiv.parentElement) === null || _a === void 0 ? void 0 : _a.parentElement;
        if (p) {
    ...
    overlay.onAdd = function () {
        ...
        let npo = new NodePositionObject();
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            serie = new dynamicgraph.ScalarTimeSeries();
            nodePositionObjectsLookupTable.push(serie);
            for (const tId in positions) {
                googleLatLng = new google.maps.LatLng(positions[parseInt(tId)].latitude(), positions[parseInt(tId)].longitude());
    ...
    map.addListener("mousemove", (ev) => {
        ...
        let minDist = 0.5 * F;
        const mouse = { x: ev.latLng.lng() * F, y: ev.latLng.lat() * F };
        let pos;
        ...
        if (intersectedNode == undefined) {
            ...
            let l;
            let sourceNPO, targetNPO;
            let sourcePoint = new google.maps.Point(0, 0);
            let targetPoint = new google.maps.Point(0, 0);
...
function createNodeLabel(npo) {
    ...
    const time = dgraph.time(npo.timeIds[0]);
    let timeLabel = "";
    if (time)
        timeLabel = " (" + toMMDDYYYY(new Date(time.unixTime())) + ")";
    return npo.node.label() + locationLabel + timeLabel;
}
let hittestRect = new google.maps.LatLngBounds();
...
function displayLocationsWindow(currentProjection, point, latLng) {
    ...
    const southwest = new google.maps.Point(point.x - hittestRadius, point.y + hittestRadius);
    const northeast = new google.maps.Point(point.x + hittestRadius, point.y - hittestRadius);
    ...
    foundLocations.forEach(function (v) {
        locationGroups.push({
            loc: v,
            nodes: foundNodes.filter(function (v2) {
    ...
    const translatePoint = projection.fromLatLngToDivPixel(latLng);
    translatePoint.x += 20;
    translatePoint.y -= 30;
...
function updateLinks(highlightId) {
    visualLinks
        ...
        let weight = linkWeightScale(d.weights(time_start, time_end).mean()) *
            LINK_WIDTH_SCALE;
...
function updateNodes(highlightId) {
    ...
    svg
        ...
        .classed("highlighted", (n) => n.node.isHighlighted() ||
    ...
    // update node labels
    vNodeLabels.attr("visibility", (n) => {
        let visible = n.node.isHighlighted() ||
        ...
        if (visible) {
            ...
            for (let i = 0; i < visibleLabels.length; i++) {
                ...
                const l1 = npo1.x + LABEL_OFFSET_X;
                const r1 = npo1.x + LABEL_OFFSET_X + npo1.node.label().length * 8;
                const t1 = npo1.y - 7;
                const b1 = npo1.y + 7;
                const l2 = npo2.x + LABEL_OFFSET_X;
                const r2 = npo2.x + LABEL_OFFSET_X + npo2.node.label().length * 8;
                const t2 = npo2.y - 7;
                const b2 = npo2.y + 7;
                // check overlap
...
function updateLocationMarkers() {
    d3.selectAll(".locationMarker").attr("transform", function (d) {
        const pos = new google.maps.LatLng(d.latitude(), d.longitude());
        const pixelpos = geoProjection.fromLatLngToDivPixel(pos);
...
function updateLinkPaths() {
    ...
    for (let i = 0; i < dgraph.links().length; i++) {
        ...
        if (targetNPO == undefined)
            targetNPO = { x: 0, y: 0 };
        link.sourceNPO = sourceNPO;
        link.targetNPO = targetNPO;
        // check for geomultilinks
        for (let j = 0; j < i; j++) {
            link2 = dgraph.link(j);
            if (linkOverlapTest(link, link2)) {
                // console.log('link overlap', i,j)
                if (link2.geoMultiLink) {
                    geoMultiLink = link2.geoMultiLink;
    ...
    for (let i = 0; i < dgraph.links().length; i++) {
        link = dgraph.link(i);
        sourceNPO = link === null || link === void 0 ? void 0 : link.sourceNPO;
        targetNPO = link === null || link === void 0 ? void 0 : link.targetNPO;
        dir = {
        ...
        if (sourceNPO.x == targetNPO.x && sourceNPO.y == targetNPO.y) {
            ...
            let multiplier = 0;
            if (link.geoMultiLink) {
                const multiLink = link.geoMultiLink;
    ...
    visualLinks.attr("d", function (d) {
        return line(d.path);
    });
}
function linkOverlapTest(l1, l2) {
...
ui.makeSlider(menuDiv, "Node Overlap", 100, MENU_HEIGHT, OVERLAP_FRACTION, -0.05, 3, function (value) {
    OVERLAP_FRACTION = value;
    updateNodePositions();
});
ui.makeSlider(menuDiv, "Edge Gap", 100, MENU_HEIGHT, LINK_GAP, 0, 10, function (value) {
    LINK_GAP = value;
    updateLinkPaths();
});
// LINK OPACITY SLIDER
menuDiv = d3.select("#menuDiv");
ui.makeSlider(menuDiv, "Link Opacity", 100, MENU_HEIGHT, LINK_OPACITY, 0, 1, function (value) {
    LINK_OPACITY = value;
    updateLinks();
});
ui.makeSlider(menuDiv, "Link Width", 100, MENU_HEIGHT, LINK_WIDTH_SCALE, 0, 10, function (value) {
    LINK_WIDTH_SCALE = value;
    updateLinks();
});
// NON-POSITIONED NODES OPACITY SLIDER
menuDiv = d3.select("#menuDiv");
ui.makeSlider(menuDiv, "Positionless Nodes", 150, MENU_HEIGHT, LINK_OPACITY, 0, 1, function (value) {
...
function updateNodeDisplacementVectors() {
    ...
    for (let i = 0; i < locations.length; i++) {
        ...
        if (localNPOs.length == 1) {
            ...
            // calculate andle
            const alpha = (Math.PI * 2) / localNPOs.length;
            let npo;
            const radius = (localNPOs.length * NODE_SIZE) / Math.PI;
...
function getNodePositionObjectsForLocation(n, long, lat) {
    const s = nodePositionObjectsLookupTable[n.id()];
    ...
    // init node positions
    const npo = new NodePositionObject();
    npo.node = n;
    npo.x = 0;
    npo.y = 0;
    npo.xOrig = 0;
...
function getStateHandler(m) {
    if (m.viewType == "map") {
        const mapNetwork = new messenger.MapControls("map", time_start.unixTime(), time_end.unixTime(), OVERLAP_FRACTION, LINK_OPACITY, NODE_UNPOSITIONED_OPACITY);
