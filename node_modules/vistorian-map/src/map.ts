...
function init() {
  ...
  overlay.onAdd = function () {
    ...
    const locationMarker: any = svg
      .selectAll(".locationMarker")
      .data(locations)
      .enter()
      .append("g")
    ...
    grad.append("svg:stop").attr("offset", 0).attr("stop-color", "#000");
    grad.append("svg:stop").attr("offset", 0.5).attr("stop-color", "#000");
    grad.append("svg:stop").attr("offset", 1).attr("stop-color", "#eee");
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (const tId in positions) {
        ...
        npo = getNodePositionObjectsForLocation(
          n,
    ...
    visualLinks.each((link: any) => {
      // get source and target NPO
      link.sourceNPO = getNodePositionObjectAtTime(
        link.source,
        link.times().get(0).id()
      );
      link.sourceNPO.outLinks.push(link);

      link.targetNPO = getNodePositionObjectAtTime(
        link.target,
        link.times().get(0).id()
      );
    ...
    vNodeLabelBackgrounds = visualNodes
      .append("rect")
    ...
    if (intersectedNode == undefined) {
      ...
      for (let i = 0; i < links.length; i++) {
        ...
        if (sourceNPO == undefined) {
          ...
          sourcePoint = new google.maps.Point(
            sourcePoint.y * F,
            sourcePoint.x * F
          ); //{ x: sourcePoint.lng() * F, y: sourcePoint.lat() * F };
        }
        ...
        if (targetNPO == undefined) {
          ...
          targetPoint = new google.maps.Point(
            targetPoint.y * F,
            targetPoint.x * F
          );
        }
...
function displayLocationsWindow(
  ...
  locations.forEach(function (v): void {
    ...
    if (hittestRect.contains(latlng)) foundLocations.push(v);
  });
  ...
  const locationGroups: Array<{
    loc: dynamicgraph.Location;
    nodes: Array<dynamicgraph.Node>;
  }> = [];
  ...
  const translatePoint: google.maps.Point =
...
function hittestNodeGeoPositions(
  bounds: google.maps.LatLngBounds
): Array<dynamicgraph.Node> {
  ...
  for (let i = 0; i < dgraph.nodes().length; i++) {
    ...
    if (n) {
      ...
      for (let j = 0; j < locations.length; j++) {
        llpos = new google.maps.LatLng(
          locations[j].latitude(),
          locations[j].longitude()
        );
        if (bounds.contains(llpos)) result.push(n); // BEFORE dgraph.node(i);
      }
    }
  }
  return result;
...
function getNodeLocation(
  node: dynamicgraph.Node
): dynamicgraph.Location | null {
...
function updateLinks(highlightId?: number) {
  visualLinks
    ...
    .attr("opacity", (d: any) => {
      ...
      if (!visible || !d.presentIn(time_start, time_end)) return 0;

      if (intersectedLink != undefined) {
        if (intersectedLink == d) return 1;
        else return LINK_OPACITY * 0.3;
      }
...
function updateNodes(highlightId?: number) {
  ...
  visualNodes.attr("opacity", (d: any) => {
    if (!d.node.isVisible()) return 0;
    return d.timeIds[0] <= time_end.id() &&
      d.timeIds[d.timeIds.length - 1] >= time_start.id()
      ? 1
  ...
  // update node labels
  vNodeLabels.attr("visibility", (n: any) => {
    let visible: any =
      n.node.isHighlighted() ||
      ...
      for (let i = 0; i < visibleLabels.length; i++) {
        ...
        if (npo1.node == npo2.node) {
          ...
          vNodeLabelBackgrounds
            ...
            .attr(
              "x",
              (d: any) => -LABEL_OFFSET_X - getTextWidth(createNodeLabel(d))
            )
            .attr("text-anchor", "end")
            .attr("visibility", "visible");
    ...
    if (visible) visibleLabels.push(n);
...
function updateLocationMarkers() {
  d3.selectAll(".locationMarker").attr("transform", function (d: any) {
...
function updateLinkPaths() {
  ...
  for (let i = 0; i < dgraph.links().length; i++) {
    ...
    // check for NPOs
    if (link) sourceNPO = link.sourceNPO;
    else sourceNPO = undefined;

    if (sourceNPO == undefined) sourceNPO = { x: 0, y: 0 };

    if (link) targetNPO = link.targetNPO;
    else targetNPO = undefined;

    if (targetNPO == undefined) targetNPO = { x: 0, y: 0 };
  ...
  for (let i = 0; i < dgraph.links().length; i++) {
    ...
    // self-links
    if (sourceNPO.x == targetNPO.x && sourceNPO.y == targetNPO.y) {
      ...
      (link as any)["path"] = [
        { x: sourceNPO.x, y: sourceNPO.y },
        {
          x: center.x + offset[0] * stretch,
          y: center.y + offset[1] * stretch,
        },
        { x: targetNPO.x, y: targetNPO.y },
      ];
    }
  ...
  visualLinks.attr("d", function (d: any) {
    return line(d.path);
  });
}
...
function linkOverlapTest(l1: any, l2: any): boolean {
  return (
    (l1.sourceNPO == l2.sourceNPO && l1.targetNPO == l2.targetNPO) ||
...
function updateEvent(m: messenger.TimeRangeMessage) {
  // BEFORE messenger.Message) {
...
function updateNodeDisplacementVectors() {
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    for (let j = 0; j < (l as any)["npos"].length; j++) {
      if (
        (l as any)["npos"][j].timeIds[0] <= time_end.id() &&
        (l as any)["npos"][j].timeIds[
          (l as any)["npos"][j].timeIds.length - 1
        ] >= time_start.id()
      ) {
...
function getNodePositionObjectsForLocation(
  n: dynamicgraph.Node,
  long: number,
  lat: number
): NodePositionObject {
...
function getNodePositionObjectAtTime(
  n: dynamicgraph.Node,
  tId: number
...
function getStateHandler(m: messenger.GetStateMessage) {
  if (m.viewType == "map") {
    const mapNetwork: messenger.NetworkControls = new messenger.MapControls(
      "map",
      time_start.unixTime(),
      time_end.unixTime(),
      OVERLAP_FRACTION,
