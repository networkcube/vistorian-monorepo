...
let time_end = times[times.length - 1];
const directed = dgraph.directed;
const nodes = dgraph.nodes().toArray();
const nodesOrderedByDegree = dgraph
    .nodes()
    .toArray()
    .sort((n1, n2) => n2.neighbors().length - n1.neighbors().length);
...
function centerViewOnSelection(m) {
    var _a;
    const selectionId = m.selectionId;
    console.log(nodes.map((n) => n.getSelections()));
    let minX = +Infinity;
    let minY = +Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    for (const node of nodes) {
        const selectionIds = node.getSelections().map((s) => s.id);
    ...
    maxX += 150;
    maxY += 150;
    const svgBB = ((_a = document.getElementById("visSvg")) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect()) || { width: 0, height: 0 };
    const xScaling = svgBB.width / (maxX - minX);
    const yScaling = svgBB.height / (maxY - minY);
    const globalZoom = Math.min(xScaling, yScaling);
    const center = [minX + (maxX - minX) / 2, minY + (maxY - minY) / 2]; // center of points in this cluster
...
/* widget/ui.js */
ui.makeSlider(menuDiv, "Link Opacity", SLIDER_WIDTH, SLIDER_HEIGHT, LINK_OPACITY, 0, 1, function (value) {
    LINK_OPACITY = value;
    updateLinks();
});
ui.makeSlider(menuDiv, "Node Opacity", SLIDER_WIDTH, SLIDER_HEIGHT, NODE_OPACITY, 0, 1, function (value) {
    NODE_OPACITY = value;
    updateNodes();
});
ui.makeSlider(menuDiv, "Node Size", SLIDER_WIDTH, SLIDER_HEIGHT, NODE_SIZE, 0.01, 30, function (value) {
    NODE_SIZE = value;
    updateNodeSize();
});
ui.makeSlider(menuDiv, "Edge Gap", SLIDER_WIDTH, SLIDER_HEIGHT, LINK_GAP, 0, 10, function (value) {
    LINK_GAP = value;
    updateLayout();
});
ui.makeSlider(menuDiv, "Link Width", SLIDER_WIDTH, SLIDER_HEIGHT, LINK_WIDTH_SCALE, 0, 10, function (value) {
    LINK_WIDTH_SCALE = value;
    updateLinks();
});
makeDropdown(menuDiv, "Labeling", ["Automatic/Importance", "Hide All", "Show All"], (selection) => {
...
function makeDropdown(d3parent, name, values, callback) {
    const s = d3parent
        ...
        .attr("onchange", "trace.event('vis_9','Node Link','selection-input_" +
        name +
        "',this.value)");
    ...
    s.on("change", () => {
        const e = document.getElementById("selection-input_" + name);
...
let mouseStart;
let mouseEnd;
let panOffsetLocal = [0, 0];
let panOffsetGlobal = [0, 0];
let isMouseDown = false;
let globalZoom = 1;
const parentSvg = d3.select("#visSvg");
...
parentSvg
    ...
    if (ev.ctrlKey) {
        ...
        selectionRect
            ...
            .attr("y", r_y)
            .attr("height", r_h);
        const selectedNodes = visualNodes.filter(function (d, i, nodes) {
            const node = nodes[i];
...
function init() {
    ...
    visualNodes = nodeLayer
        ...
        .enter()
        .append("path")
        .attr("d", (n) => d3.symbol().type(getNodeShape(n))())
        .attr("class", "nodes")
    ...
    visualLinks = linkLayer
      ...
      */
        ...
        for (let j = 0; j < selections.length; j++) {
            if (selections[j] == currentSelection) {
                messenger.selection("remove", { links: [d] });
                return;
            }
        }
        messenger.selection("add", { links: [d] });
    });
    updateLinks();
    updateNodes();
    updateNodeSize();
...
function handleLasso() {
    const lasso_draw = function () {
        const newElementCompound = new utils.ElementCompound();
        newElementCompound.nodes = l.notPossibleItems().nodes().map((n) => n.__data__);
        messenger.highlight("set", newElementCompound, "NODE_LASSO");
    };
    const lasso_end = function () {
        const newElementCompound = new utils.ElementCompound();
        newElementCompound.nodes = l.selectedItems().nodes().map((n) => n.__data__);
        messenger.highlight("set", newElementCompound, "NODE_LASSO");
    };
    const l = lasso()
...
function updateLayout() {
    ...
    nodeHighlights.attr("transform", function (d) {
        return "translate(" + d.x + "," + d.y + ")";
    });
    nodeLabels
        .attr("x", (d) => d.x + OFFSET_LABEL.x)
        .attr("y", (d) => d.y + OFFSET_LABEL.y);
    nodeLabelOutlines
        .attr("x", (d) => d.x + OFFSET_LABEL.x)
        .attr("y", (d) => d.y + OFFSET_LABEL.y);
    // update link positions
    calculateCurvedLinks();
    visualLinks.attr("d", (d) => lineFunction(d.path));
...
function getStateHandler(m) {
    if (m.viewType == "nodelink") {
        const nlNetwor = new messenger.NodeLinkControls("nodelink", time_start.unixTime(), time_end.unixTime(), globalZoom, panOffsetLocal, panOffsetGlobal, LINK_OPACITY, NODE_OPACITY, NODE_SIZE, LINK_GAP, LINK_WIDTH_SCALE, LABELING_STRATEGY);
...
function updateNodeSize() {
    visualNodes
        .attr("d", (n) => d3.symbol().size(getNodeRadius(n)).type(getNodeShape(n))());
    nodeHighlights
        .attr('r', (n) => getNodeRadius(n) / 10 + 5);
}
function updateNodes(highlightId) {
    ...
    nodeLabelOutlines.attr("visibility", (d) => {
        // e.isHighlighted() ||
        // e.links().highlighted().length > 0 ||
        // hiddenLabels.indexOf(e) == -1 ||
        // (LABELING_STRATEGY == 3 && e.neighbors().highlighted().length > 0)
...
function updateLinks(highlightId) {
    ...
    visualLinks
        ...
        const w = linkWeightScale(d.weights(time_start, time_end).mean()) *
            LINK_WIDTH_SCALE;
...
function calculateCurvedLinks() {
    let dir, offset, offset2, nodePair;
    let links;
        ...
        if (nodePair) {
            ...
            if (nodePair.links().length < 2) {
                if (nodePair.source != nodePair.target) {
                    nodePair.links().toArray()[0]["path"] = [
                        { x: nodePair.source.x, y: nodePair.source.y },
                        { x: nodePair.target.x, y: nodePair.target.y },
                    ];
                }
                else {
                    ...
                    nodePair.links().toArray()[0]["path"] = [
                        { x: nodePair.source.x, y: nodePair.source.y },
                        {
                            x: nodePair.source.x + 10,
                            y: nodePair.source.y,
                        },
                        {
                            x: nodePair.source.x + 10,
                            y: nodePair.source.y - 10,
                        },
                        {
                            x: nodePair.source.x,
                            y: nodePair.source.y - 10,
                ...
                if (nodePair.source == nodePair.target) {
                    ...
                    for (let j = 0; j < links.length; j++) {
                        links[j]["path"] = [
                            ...
                            {
                                x: nodePair.source.x,
                                y: nodePair.source.y - minGap - j * LINK_GAP,
                            },
                            {
                                x: nodePair.source.x + minGap + j * LINK_GAP,
                                y: nodePair.source.y - minGap - j * LINK_GAP,
                            },
                            {
                                x: nodePair.source.x + minGap + j * LINK_GAP,
                    ...
                    dir = {
                        x: nodePair.target.x - nodePair.source.x,
                        y: nodePair.target.y - nodePair.source.y,
                    };
                    // normalize
...
window.addEventListener("message", (event) => {
    if (event.data.type === "clearSelections") {
        ...
        const clusterSelection = messenger.createSelection('node', `cluster-${addedSelections.length + 1}`);
