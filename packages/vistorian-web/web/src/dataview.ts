...
if (files) files.addEventListener("change", getFileInfos, false);
...
export function init() {
  ...
  if (networkids.length > 0) showNetworkTables(networkids[0]);
}
...
export function loadVisualizationList() {
  ...
  visualizations.forEach(function (v) {
    $("#visualizationList").append(
      '<li class="visLink" title="Show ' +
        v[0] +
        ' visualization.">\
                        <button onclick="window.exports.networkcube.dataview.loadVisualization(\'' +
        v[1] +
        "');trace.event('vis_1','data view','Vis Creation Link Clicked','" +
        v[1] +
        '\');" class="visbutton hastooltip">\
                            <img src="../static/figures/' +
        v[1] +
        '.png" class="visicon"/>\
                            <p>' +
        v[0] +
  ...
  $("#visualizationList").append(
    '<li class="visLink" title="Show matrix and node-link split-view.">\
...
export function loadTableList() {
  ...
  tableNames.forEach((t) => {
    let shownName = t;
    if (t.length > 30) shownName = t.substring(0, 30) + "..";
    $("#tableList").append(
      "<li>\
            <a onclick=\"showSingleTable('" +
        t +
        '\')"  class="underlined">' +
        shownName +
        '.csv</a>\
            <img class="controlIcon" title="Delete this table." src="../static/logos/delete.png" onclick="deleteCurrentNetworkBookmarks();window.exports.networkcube.dataview.removeTable(\'' +
        t +
        "');trace.event('dat_4','data view','selected table','deleted')\"/>\
        </li>"
    );
  });
}
...
export function loadNetworkList() {
  ...
  networkNames.forEach((t: any) => {
    ...
    $("#networkList").append(
      "\
            <li>\
                <a onclick=\"window.exports.networkcube.dataview.showNetworkTables('" +
        network.id +
        '\')"  class="underlined">' +
        networkDisplayName +
        '</a>\
                <img class="controlIcon" title="Delete this network." src="../static/logos/delete.png" onclick="window.exports.networkcube.dataview.removeNetwork(\'' +
        network.id +
        "');trace.event('dat_4','data view','selected network','deleted')\"/>\
                <img class=\"controlIcon\" title=\"Download this network in .vistorian format.\" src=\"../static/logos/download.png\" onclick=\"window.exports.networkcube.dataview.exportNetwork('" +
        network.id +
...
export function setNodeTable(list: any) {
  ...
  if (tableName != "---") {
    const table: vistorian.VTable = storage.getUserTable(
      tableName,
      SESSION_NAME
    );
...
export function saveCurrentNetwork(
  ...
  if (currentNetwork.userNodeSchema) {
    if (currentNetwork.userNodeSchema.time != -1) {
      currentNetwork.timeFormat = $(
        "#timeFormatInput_" + currentNetwork.userNodeSchema.name
      ).val();
    }
  }
  if (currentNetwork.userLinkSchema) {
    if (currentNetwork.userLinkSchema.time != -1) {
      currentNetwork.timeFormat = $(
        "#timeFormatInput_" + currentNetwork.userLinkSchema.name
      ).val();
    }
  }
...
export function showNetworkTables(networkId: number) {
  ...
  $("#locationtableSelect").append(
    '<option class="tableSelection">---</option>'
  );
  ...
  tables.forEach((t) => {
    $("#nodetableSelect").append(
      '<option value="' + t.name + '">' + t.name + "</option>"
    );
    $("#linktableSelect").append(
      '<option value="' + t.name + '">' + t.name + "</option>"
    );
    $("#locationtableSelect").append(
      '<option value="' + t.name + '">' + t.name + "</option>"
  ...
  if (currentNetwork.userLocationTable) {
    showTable(
      currentNetwork.userLocationTable,
      "#locationTableDiv",
      true,
      currentNetwork.userLocationSchema
    );
  ...
  $("#tileViewLink").attr(
    "href",
    "sites/tileview.html?session=" +
      SESSION_NAME +
      "&datasetName=" +
      currentNetwork.name.split(" ").join("___")
  );
  $("#mat-nlViewLink").attr(
    "href",
    "sites/mat-nl.html?session=" +
      SESSION_NAME +
      "&datasetName=" +
      currentNetwork.name.split(" ").join("___")
  );
...
export function updateNetworkStatusIndication() {
  if (currentNetwork.ready) {
    $("#networkStatus")
      .text(
        "Network ready for visualization. Select a visualization from the menu on the top."
      )
      .css("color", "#fff")
      .css("background", "#6b6");
  } else if (
    "userNodeTable" in currentNetwork &&
    currentNetwork.userNodeTable &&
    currentNetwork.userNodeTable.data.length == 1
  ) {
    $("#networkStatus")
      .text(
        "Network not ready for visualization. Uploaded node table is empty."
      )
      .css("background", "#f63")
      .css("color", "#fff");
  } else if (
    "userLinkTable" in currentNetwork &&
    currentNetwork.userLinkTable &&
    currentNetwork.userLinkTable.data.length == 1
  ) {
    $("#networkStatus")
      .text(
        "Network not ready for visualization. Uploaded link table is empty."
      )
      .css("background", "#f63")
      .css("color", "#fff");
  } else {
    $("#networkStatus")
      .text(
        "Network not ready for visualization. Table or Schema specifications missing."
      )
      .css("background", "#f63")
      .css("color", "#fff");
  }
...
export function showTable(
  table: vistorian.VTable,
  elementName: string,
  isLocationTable: boolean,
  schema?: vistorian.VTableSchema
) {
  ...
  if (data.length > DATA_TABLE_MAX_LENGTH) {
    const info = $(
      "<p>Table shows first 200 rows out of " +
        data.length +
        " rows in total.</p>"
    );
  ...
  const csvExportButton = $(
    '<button class="menuButton exportButton" onclick="window.exports.networkcube.dataview.exportCurrentTableCSV(\'' +
      table.name +
      "');trace.event('dat_8', 'data view', 'export as CSV file','" +
      elementName +
  ...
  for (let c = 0; c < data[0].length; c++) {
    const td = $("<th></th>")
      ...
      .attr("onclick", "trace.event('dat_16','data view','column','Sorted')");
    tr.append(td);
    td.html(data[0][c]);
  }
  ...
  for (let r = 1; r < Math.min(data.length, DATA_TABLE_MAX_LENGTH); r++) {
    if (elementName == "#nodeTableDiv") {
      tr = $("<tr></tr>").addClass("tablerow").attr({
        onmouseover:
          "window.exports.networkcube.dataview.nodeRowMouseOver(this)",
        id: r,
      });
    } else if (elementName == "#linkTableDiv") {
      tr = $("<tr></tr>").addClass("tablerow").attr({
        onmouseover:
          "window.exports.networkcube.dataview.linkRowMouseOver(this)",
        id: r,
      });
    } else {
    ...
    for (let c = 0; c < data[r].length; c++) {
      ...
      if (isLocationTable && data[0][c] == "User Name") {
        ...
        td = $(
          "<td onchange=\"trace.event('dat_10', 'data view','" +
            elementName +
            " table ', 'cell edited' )\"></td>"
  ...
  if (schema) {
    ...
    for (let i = 0; i < table.data[0].length; i++) {
      cell = $(
        '<th class="schemaCell" id="schemaCell_' +
          schema.name +
          "_" +
          i +
          '"></th>'
      ...
      select = $(
        '<select class="schemaSelection" onchange="window.exports.networkcube.dataview.schemaSelectionChanged(this.value, ' +
          i +
          " , '" +
          schema.name +
          "');trace.event('dat_11','data view','Column Data Type Specified', this.value);\"></select>"
      );
      cell.append(select);
      select.append("<option>(Not visualized)</option>");

      for (const field in schema) {
        if (field == "name" || field == "constructor" || field == "timeFormat")
          continue;
        ...
        switch (field) {
          ...
          case "location_source":
            fieldName = "Location Source Node";
            break;
          case "location_target":
            fieldName = "Location Target Node";
            break;
          case "linkType":
            fieldName = "Link Type";
            break;
        ...
        if (field == "directed") {
          option = $(
            "<option value=" +
              field +
              " class=directionField style=display:block>" +
              fieldName +
              "</option>"
          );
        ...
        if ((schema as any)[field] == i) {
          ...
          if (field == "time") {
            ...
            timeFormatInput = $(
              '<span class="nobr"><input title="Enter a date pattern" type="text" size="12" id="timeFormatInput_' +
                schema.name +
                '" ' +
                val +
                '></input><a href="http://momentjs.com/docs/#/parsing/string-format/" target="_blank" title="Details of the date pattern syntax"><img src="../static/logos/help.png" class="inlineicon"/></a></span>'
            );
...
//
export function schemaSelectionChanged(
  field: string,
  columnNumber: number,
  schemaName: string,
  parent: HTMLElement
  ...
  for (const field2 in (currentNetwork as any)[schemaName]) {
    if (
      field2 == "relation" &&
      (currentNetwork as any)[schemaName][field2].indexOf(columnNumber) > -1
    ) {
      if (field == "(Not visualized)") {
        (currentNetwork as any)[schemaName][field2].splice(
          (currentNetwork as any)[schemaName][field2].indexOf(columnNumber),
          1
        );
      } else {
        // JSB: This seems to be a no-op?
        const arr: any = (currentNetwork as any)[schemaName][field];
        (currentNetwork as any)[schemaName][field2].slice(
          arr.indexOf(columnNumber),
          0
        );
      }
    } else {
  ...
  if (field == "location_source" || field == "location_target") {
    extractLocations();
...
export function getFileInfos(e: any) {
  ...
  for (let i = 0, f; i < files.length; i++) {
    ...
    if (f.name.split(".")[1] != "csv") {
      showMessage(
        "Uploaded file is not a .csv file. Please chose another file.",
        4000
      );
      return;
    } else {
...
export function uploadNodeTable(e: any) {
  ...
  if (checkFileType(filesToUpload)) {
    uploadFiles(() => {
      ...
      $("#nodetableSelect").append(
        '<option value="' + lastTable.name + '">' + lastTable.name + "</option>"
      );
...
export function exportCurrentTableCSV(tableName: string) {
  ...
  if (tableName) {
    if (
      currentNetwork.userLinkTable &&
      currentNetwork.userLinkTable.name == tableName
    )
      table = currentNetwork.userLinkTable;
    else if (
      currentNetwork.userNodeTable &&
      currentNetwork.userNodeTable.name == tableName
    )
      table = currentNetwork.userNodeTable;
    else if (
      currentNetwork.userLocationTable &&
      currentNetwork.userLocationTable.name == tableName
    )
...
export function replaceCellContents(tableId: any) {
  ...
  let table: vistorian.VTable | undefined = storage.getUserTable(
    tableId,
    SESSION_NAME
  );
  if (table == undefined) {
...
export function directedCheckboxClick() {
  ...
  const directionFields = document.getElementsByClassName(
    "directionField"
...
export function extractLocations() {
  ...
  if (currentNetwork.userLocationTable == undefined) {
    ...
    currentNetwork.userLocationTable = new vistorian.VTable(
      tableName + "-locations",
      []
    );
    currentNetwork.userLocationSchema = new datamanager.LocationSchema(
  ...
  if (currentNetwork.userLinkSchema && currentNetwork.userLinkTable) {
    for (let row = 1; row < currentNetwork.userLinkTable.data.length; row++) {
      if (
        ...
        createLocationEntry(
          currentNetwork.userLinkTable.data[row][
            currentNetwork.userLinkSchema.location_target
          ],
  ...
  if (currentNetwork.userNodeSchema && currentNetwork.userNodeTable) {
    for (let row = 1; row < currentNetwork.userNodeTable.data.length; row++) {
      if (
        currentNetwork.userNodeSchema.location &&
        currentNetwork.userNodeSchema.location > -1
      ) {
        createLocationEntry(
          currentNetwork.userNodeTable.data[row][
            currentNetwork.userNodeSchema.location
          ],
          currentNetwork.userLocationTable.data
        );
  ...
  console.log(
    "\tlocations:",
    currentNetwork.userLocationTable.data.length,
    currentNetwork.userLocationTable.data
  );
...
export function createLocationEntry(name: string, locationTableData: any[]) {
  ...
  // check validity
  if (name == undefined || name.length == 0 || !name.replace(/\s/g, "").length)
    // contains only whitespace
    return;

  // check if location entry exists already.
  for (let i = 0; i < locationTableData.length; i++) {
    if (locationTableData[i][1] == name) return;
  }

  locationTableData.push([
    locationTableData.length - 1,
    name,
    name,
    undefined,
    undefined,
  ...
  if (currentNetwork.userLocationTable) {
    updateLocationCoordinates(
      ...
      function () {
        // showNetworkTables(currentNetwork.id);
        showTable(
          currentNetwork.userLocationTable as vistorian.VTable,
          "#locationTableDiv",
          true,
          currentNetwork.userLocationSchema
        );
...
export function exportNetwork(networkId: number) {
  vistorian.exportNetwork(
    storage.getNetwork(networkId.toString(), SESSION_NAME)
  );
}
...
export function updateLocationCoordinates(
  ...
  for (let i = 1; i < userLocationTable.data.length; i++) {
    getOpenStreetMapCoordinatesForLocation(
      i,
      userLocationTable.data[i][locationSchema.geoname],
      userLocationTable,
      locationSchema
    );
  }
...
export function getOpenStreetMapCoordinatesForLocation(
  index: number,
  geoname: string,
    ...
    const xhr: any = $.ajax({
      ...
      .done(function (data, text, XMLHttpRequest) {
        ...
        const userLocationLabel =
          locationTable.data[rowIndex][locationSchema.label];
