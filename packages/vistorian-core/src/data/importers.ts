...
// TODO: parse functions shoull accept a name not a url

export function parseDyson(data: any, dataName: string): DataSet {
  // create node table
  const nodeTable = [];
...
export function loadDyson(
  ...
  d3.json(url).then((data: any) => callback(parseDyson(data, url)));
}

/** Loads a .csv file from the indicated url*/
// does not check for locations

export function parseLinkTable(
  text: string,
  dataName: string,
  linkSchema: LinkSchema,
  delimiter: string,
...
export function loadLinkTable(
  ...
  fetch(url)
    ...
    .then((text) =>
      callBack(parseLinkTable(text, url, linkSchema, delimiter, timeFormat))
    );
}

export function parseXML(data: any, dataName: string): DataSet {
  // "application/xml",
...
export function loadXML(
  ...
  const dataName = url.split("=")[0];
  d3.xml(url).then((data: any) => callBack(parseXML(data, dataName)));
}

/** Loads a .json file from the indicated url.
 * The json must have a 'nodes/vertices' and a 'links/edge/connections/relations' array.
 */
export function parseJson(data: any, dataName: string): DataSet {
  // fill edge table
...
export function loadJson(
  ...
  const name = dataName ? dataName : url.split("=")[0];
  d3.json(url).then((data: any) => callBack(parseJson(data, name)));
}

export function parseJsonList(data: any, dataName: string): DataSet {
...
export function loadJsonList(
  ...
  const dataName = url.split("=")[0];
  d3.json(url).then((data: any) => callBack(parseJsonList(data, dataName)));
}

export function parseNCube(data: any, dataName: string): DataSet {
  const nodeTable: any[][] = [];
  const linkTable: any[][] = [];
...
export function loadNCube(
  ...
  const dataName = url.split("=")[0];
  d3.json(url).then((data: any) => callBack(parseNCube(data, dataName)));
}

export function parsePajek(data: any, dataName: string): DataSet {
...
export function loadPajek(
  ...
  fetch(url)
    .then((response) => response.text())
    .then((data) => callBack(parsePajek(data, dataName)));
}

export function parseMat(data: string, dataName: string): DataSet {
  const lines = data.split("\n");
  const nodeTable = [];
  const nodeSchema = { id: 0, label: 1 };
  const linkTable = [];
...
export function loadMat(
  ...
  fetch(url)
    .then((response) => response.text())
    .then((data) => callBack(parseMat(data, dataName)));
}

export function parseGEDCOM(dataText: string, dataName: string): DataSet {
  const nodeTable: any[] = [];
  const nodeSchema = { id: 0, label: 1, nodeType: 2 };
  const linkTable: any[] = [];
  const linkSchema = { id: 0, source: 1, target: 2 };
...
export function loadGEDCOM(
  ...
  fetch(url)
    .then((response) => response.text())
    .then((data) => callBack(parseGEDCOM(data, dataName)));
}

export function parseLinkList(data: string, dataName: string): DataSet {
  const lines = data.split("\n");
  const nodeTable = [];
  const nodeSchema = { id: 0, label: 1 };
  const linkTable = [];
...
export function loadLinkList(
  ...
  fetch(url)
    .then((response) => response.text())
    .then((data) => callBack(parseLinkList(data, dataName)));
}

export function parseMatrix(data: string, dataName: string): DataSet {
  const lines = data.split("\n");
  const nodeTable = [];
  const nodeSchema = { id: 0, label: 1 };
  const linkTable = [];
...
export function loadMatrix(
  ...
  fetch(url)
    .then((response) => response.text())
    .then((data) => callBack(parseMatrix(data, dataName)));
}
