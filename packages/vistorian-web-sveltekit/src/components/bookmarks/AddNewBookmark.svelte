<script lang="ts">
	...
	function updateLocalStorage(bookmarksArray) {
		...
		document.getElementById('lbl_changesState').innerText =
			'No of Bookmarks: ' + bookmarksArray.length + ' - Changes saved'; // TODO: set this properly
	}
	...
	function stateCreatedHandler(m) {
		...
		if (m.isNewBookmark) {
			...
			if (
				m.state.networkType == bookmarks[m.bookmarkIndex].viewType ||
				m.state.networkType == 'nodelink'
			)
				bookmarks[m.bookmarkIndex].controlsValues[0] = m.state;
			else if (m.state.networkType == 'matrix')
			...
			else if (m.state.networkType == 'map') bookmarks[m.bookmarkIndex].controlsValues[3] = m.state;
		}

		localStorage.setItem('vistorianBookmarks', JSON.stringify(bookmarks));

		if (
			(m.typeOfMultiView == 'mat-nl' || m.typeOfMultiView == 'tileview') &&
			m.state.networkType == 'nodelink'
		)
	...
	const addBookmark = () => {
		...
		parentUrlTxt = parentUrlTxt.substring(
     ...
     */
		...
		newBookmark.updateBookmarkType(
			selectedUse.id === 'Other' ? otherExplanation : selectedUse.label
		);
	...
	let bookmarkLabel = 'Update bookmark label here...',
		notesText = 'Enter your notes...';
</script>

<div>
	<Modal isOpen={open} {toggle}>
		<ModalHeader {toggle}
			><h5 style="color: orange;">Add New Bookmark & Log your Activity</h5></ModalHeader
		>
		<ModalBody>
			<Form>
				<FormGroup>
				...
				<FormGroup>
					...
					<div>
						{#each uses as use}
							<Button
								color={use.id === selectedUse.id ? 'success' : 'secondary'}
						...
						{#if selectedUse.id === 'AnalyzeData'}
							<AnalysisType bind:selectedActivityIndexes />
						{/if}
					...
					<FormText color="muted">Only bookmarks types are shared with VistorianLab.</FormText>
				</FormGroup>
			</Form>
		</ModalBody>
		<ModalFooter>
