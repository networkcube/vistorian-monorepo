...
*/
    ...
    const exportBookmarks = () => {
        trace.event('bkm_9', window.parent.location.pathname, 'bookmark_tool', 'Bookmarks File Exported');
    ...
    const importBookmarks = () => {
        trace.event('bkm_10', window.parent.location.pathname, 'bookmark_tool', 'Bookmarks File Imported');
    ...
    const clearBookmarks = () => {
        trace.event('bkm_10', window.parent.location.pathname, 'bookmark_tool', 'Bookmarks File Cleared');
...
<main>
    <div id="logbook_container">
        ...
        {#if state === "addingBookmark"}
            <AddNewBookmark bind:bookmarks={bookmarks}/>
        {/if}
        ...
        <FormGroup>
            <Input
                    ...
                    on:click={() => trace.event('bkm_7', ' show all bookmarks (view)', 'selected', window.parent.location.pathname)}
                    value="all"
            ...
            <Input
                    ...
                    type="radio"
                    bind:group={whichBookMarksToShow}
                    on:click={() => trace.event('bkm_7', ' show current view bookmarks', 'selected', window.parent.location.pathname)}
                    value="specific"
        ...
        <Button block outline
                on:click={() => {location.href='mailto:vistorian@inria.fr?subject=[Vistorian] Consultation Request&body=Hi, kindly I would like to arrange a 10-minutes meeting to consult about my data\'s visualization. Thank you.'; trace.event('com_3', window.parent.location.pathname, 'bookmark_tool', 'consult_team');}}>
            <u>E</u>mail Us & Consult an Expert
        </Button>

        <Button block outline
                on:click={() => {location.href='mailto:vistorian@inria.fr?subject=[Vistorian] Report Technical Bug &body=Hello,\n I am facing a technical problem and would like to request your help. The description of my problem is :\n';trace.event('com_4', window.parent.location.pathname, 'bookmark_tool', 'ReportTechBug');}}>
