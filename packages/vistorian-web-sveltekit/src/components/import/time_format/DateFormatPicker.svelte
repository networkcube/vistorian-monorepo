<script>
  ...
  const addDirective = async (directiveCode) => {
    ...
    document.getElementById("date-form-input").setSelectionRange(pos + directiveCode.length, pos + directiveCode.length);
...
<p><b>Type in the input box to edit the format string, or click on a button to append the corresponding directive. You
  can manually enter character such as colons, hyphens, spaces, or parentheses.</b></p>
...
{#each Object.keys(directives) as t}
  ...
  <DirectiveGroup directives={directives[t]} groupTitle={t} filter={filter} addDirective={addDirective} />
