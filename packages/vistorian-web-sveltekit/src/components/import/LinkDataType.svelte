...
<Card class="mb-3" style="width: 50%">
  <CardBody>
    <h2>
      How are links or edges represented in your network?
    </h2>

    <input type="radio" bind:group={linkDataType} value={"linkTable"} />
    I have a file containing one row per <i>link</i>, listing pairs of nodes that are linked (a <i>link file</i>)
    ...
    I have a file containing one row per <i>node</i>, with columns listing the other nodes it is linked to (a <i>node
    file</i>)
  ...
  <CardFooter>
    ...
    <Button style="float: right" on:click={() => stage=next_stage()} disabled={!linkDataType}>Next</Button>
  </CardFooter>
</Card>
