...
<Card class="mb-3" style="width: 50%">
  <CardBody>
    ...
    {#if config.selectedFile}
      <br />
      <FieldSelector selectedFile={config.selectedFile} label={"Node"} bind:selectedField={config.fieldNode}
                     required={true} />
      ...
      <ul>
        {#each config.fieldRelations as fieldRelation, i}
          <li>
            ...
            <FieldSelector selectedFile={config.selectedFile} label={"Column in CSV file"}
  ...
  <CardFooter>
    ...
    <Button style="float: right" on:click={() => stage=next_stage()} disabled={!ready}>Next</Button>
  </CardFooter>
</Card>
