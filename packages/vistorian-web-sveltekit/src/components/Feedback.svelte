<script>
    ...
    */
		...
		const selectedUses = uses
			.filter((u) => values[u.id])
			.map((u) => u.id)
			.join(', ');
...
<main>
	<Modal isOpen={status === 'editing' || status === 'submitted'} toggle={cancelFeedback}>
		...
		<ModalBody>
			<div>
				{#if status === 'submitted'}
					<h6>Feedback sent! Thank you for helping us!</h6>
				{:else}
					<h6>Tell us how you use the vistorian in this moment</h6>

					{#each uses as use}
						<Button
							color={values[use.id] ? 'success' : 'secondary'}
							style="margin-left: 5px; margin-bottom: 5px"
							id={use.id}
							on:click={(ev) => {
								values[use.id] = !values[use.id];
								ev.preventDefault();
							}}
