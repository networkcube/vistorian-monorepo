<script>
  ...
  const clearCache = () => {
    ...
    const msg = "Are you sure you want to clear the cache? This will remove ALL your networks and tables from your browser cache. FOREVER.";
    if  (!confirm(msg)){
      return;
    }
  ...
  <div style="display: grid; grid-template-columns: 500px 1fr;">
    ...
    <div id="lists" xs="2">
      ...
      <div id="networkListDiv">
        <h2>My Networks</h2>
        <p>These are the networks that you have already created. Click any of them then select a visualization
          below. </p>

        <ul id="networkList" class="nointent" />

        {#each networks as network}
          <li style={`font-weight: ${selectedNetwork && selectedNetwork.id === network.id ? 'bold' : 'normal'}`}>
            <a on:click={() => selectedNetwork = network}>{truncateName(network.name)}</a>
            <img class="controlIcon" title="Delete this network." src="../logos/delete.png"
                 on:click={() => deleteNetwork(network)} />
            <img class="controlIcon" title="Download this network in .vistorian format."
                 src="../logos/download.png" on:click={() => exportNetwork(network)} />
          </li>
        {/each}
