...
function binaryTreeWalk(currentNode: any, tree: any, depth: any, doLeafAction: any, doDownAction: any, doUpAction: any) {
...
function addClosestPairByCommunity(tree: any, deltaQ: any, a: any) {
    ...
    for (var hash in deltaQ) {
        ...
        var i = Number(keys[0]);
        var j = Number(keys[1]);
        if (!(H as any)[i]) { (H as any)[i] = []; }
        if (!(H as any)[j]) { (H as any)[j] = []; }
        (H as any)[i].push({ "dQ": dQ, "i": i, "j": j });
        (H as any)[j].push({ "dQ": dQ, "i": i, "j": j });
    }
    ...
    for (var i = 0; i < n; i++) {
        if ((H as any)[i]) {
            (H as any)[i].sort(function (a: any, b: any) {
                return b.dQ - a.dQ;
            });  // Full sort, overkill but native & clean
            //  The [0] element in each H array now has the largest deltaQ for that row
            if ((H as any)[i][0].dQ > Hmax.dQ) {
                Hmax.dQ = (H as any)[i][0].dQ;
                Hmax.i = (H as any)[i][0].i;
                Hmax.j = (H as any)[i][0].j;
            }
        }
    }
    ...
    for (var k = 0; k < n; k++) {
        if (k != Hmax.i && k != Hmax.j && (H as any)[k]) {  //  H[k]!=null => node still in play
    ...
    for (var hash in deltaQ) {
        if (deltaQ[hash]) {
            (dQcopy as any)[hash] = deltaQ[hash];
            ndq++;
        }
    }
...
function buildTreeByCommunities(dataObj: any, showNotes?: any) {
    ...
    for (var hash in dataObj.distances) {
        ...
        var j = Number(keys[1]);
        if (dataObj.useWeights) {
            (deltaQ as any)[hash] = 2.0 * inv2m * dataObj.distances[hash] - 2.0 * inv2m * inv2m * k[i] * k[j];
        } else {
            (deltaQ as any)[hash] = 2.0 * (inv2m - k[i] * k[j] * inv2m * inv2m);  // 2x assures identical Q for unweighted datasets
        }
    }
    var a = [];
    for (var i = 0; i < n; i++) {
    ...
    var Q = 0.0;
    var maxQ = -Infinity;
    var dQobj: any = { "value": 0, "array": deltaQ };
    ...
    binaryTreeWalk(root, tree, 0, function (currentNode: any, tree: any, depth: any) {
...
function findSplits(treeObj: any) {
    ...
    var members = "";
    var tracker = 0;
    binaryTreeWalk(treeObj.root, treeObj.tree, 0, function (node: any, tree: any, depth: any) {
        if (breakNext) {
            g += 1;
        ...
        function (node: any, tree: any, depth: any) {
...
function findSubCommunities(treeObj: any, depth: any, prevGroup: any): any {
    if (!treeObj) { return; }
...
export function cluster(nodes: any, edges: any, clusterAttr?: any, edgesCountAttr?: any) {
    var dataObj = {},
        treeObj, groups = [], i;
    ...
    if (edges.length > 0 && typeof (edges[0].source) === "object") {
        linksForClustering = edges.map(function (d: any) {
    ...
    (dataObj as any).method = "newman";
    (dataObj as any).useWeights = true;
    (dataObj as any).names = nodes.map(function (d: any, i: any) { return "" + i; });
    //console.log('names', names)

    // dataObj.names = nodes;
    (dataObj as any).distances = {};
    linksForClustering = linksForClustering.filter(function (d: any) { return d.sourceId !== d.targetId; }); //avoid loops
    linksForClustering.forEach(function (d: any) {
        var hash = Math.min(d.sourceId, d.targetId) + "~" + Math.max(d.sourceId, d.targetId); // hash always has left key < right key})
        (dataObj as any).distances[hash] = +d.count;
    });
    ...
    i = 0;
    groups.forEach(function (d: any, i: any) {
        function assignToCluster(ele: any, i: any) {
...
function removeDummyMetaNode(groups: any) {
    var i, ele;
