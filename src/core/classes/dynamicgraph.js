...
(function (networkcube) {
    ...
    var DynamicGraph = /** @class */ (function () {
        function DynamicGraph() {
            ...
            // highlighted objects
            this.highlightArrays = new nt_u.networkcube.IDCompound();
        ...
        DynamicGraph.nodeArrayReviver = function (k, v, s) {
            switch (k) {
                ...
                case 'inLinks':
                case 'links':
                    return nt_u.networkcube.copyTimeSeries(v, function () { return new nt_q.networkcube.ArrayTimeSeries(); });
                case 'outNeighbors':
                case 'inNeighbors':
                case 'neighbors':
                    return nt_u.networkcube.copyTimeSeries(v, function () { return new nt_q.networkcube.ArrayTimeSeries(); });
                case 'locations':
                    return nt_u.networkcube.copyTimeSeries(v, function () { return new nt_q.networkcube.ScalarTimeSeries(); });
                default:
                    return v;
            }
        };
        ...
        DynamicGraph.prototype.initDynamicGraph = function (data) {
            ...
            var attribute;
            var time;
            console.assert(data.nodeTable.length == 0 || nt_u.networkcube.isValidIndex(data.nodeSchema.id), 'either there is no nodeTable data, or we have a schema for the nodetable');
            ...
            for (var i = 0; i < data.nodeTable.length; i++) {
                ...
                if (nodeId_table == -1) {
                    ...
                    this.nodeArrays.outLinks.push(new nt_q.networkcube.ArrayTimeSeries());
                    this.nodeArrays.inLinks.push(new nt_q.networkcube.ArrayTimeSeries());
                    this.nodeArrays.links.push(new nt_q.networkcube.ArrayTimeSeries()); // both, in and out
                    this.nodeArrays.outNeighbors.push(new nt_q.networkcube.ArrayTimeSeries());
                    this.nodeArrays.inNeighbors.push(new nt_q.networkcube.ArrayTimeSeries());
                    this.nodeArrays.neighbors.push(new nt_q.networkcube.ArrayTimeSeries());
                    this.nodeArrays.selections.push([]);
                    this.nodeArrays.filter.push(false);
                    this.nodeArrays.locations.push(new nt_q.networkcube.ScalarTimeSeries());
            ...
            console.assert(data.linkTable.length == 0 || (nt_u.networkcube.isValidIndex(data.linkSchema.id)
                && nt_u.networkcube.isValidIndex(data.linkSchema.source)
                && nt_u.networkcube.isValidIndex(data.linkSchema.target)), 'either there are no links, or the linkschema is defined');
            for (var i = 0; i < data.linkTable.length; i++) {
                ...
                if (this.linkArrays.id.indexOf(linkId) == -1) {
                    ...
                    this.linkArrays.weights[linkId] = new nt_q.networkcube.ScalarTimeSeries();
        ...
        DynamicGraph.prototype.selection = function (action, idCompound, selectionId) {
            ...
            var self = this;
            if (action == 'set') {
                var c = new nt_u.networkcube.IDCompound();
        ...
        DynamicGraph.prototype.deleteSelection = function (selectionId) {
            ...
            // =======
            // remove 
            var idCompound = new nt_u.networkcube.IDCompound();
    ...
    var AttributeArray = /** @class */ (function () {
    ...
    var NodeArray = /** @class */ (function (_super) {
        ...
        function NodeArray() {
            ...
            _this.outLinks = []; // contains link ids only, since every GRAPH has its own EDGE object instance
            _this.inLinks = []; // contains link ids only, since every GRAPH has its own EDGE object instance
            _this.links = [];
            _this.outNeighbors = []; // contains node ids only, since every GRAPH has its own NODE object instance
            _this.inNeighbors = []; // contains node ids only, since every GRAPH has its own NODE object instance
            _this.neighbors = [];
            _this.selections = [];
            _this.attributes = []; // arbitrary attributes (key -> value)
            _this.locations = [];
            _this.filter = [];
            _this.nodeType = [];
            return _this;
    ...
    var LinkArray = /** @class */ (function (_super) {
        ...
        function LinkArray() {
            ...
            _this.selections = [];
            _this.filter = [];
            _this.attributes = new Object; // arbitrary attributes (key -> value)
            return _this;
        }
        return LinkArray;
    }(AttributeArray));
    networkcube.LinkArray = LinkArray;
    var NodePairArray = /** @class */ (function (_super) {
    ...
    var TimeArray = /** @class */ (function (_super) {
        __extends(TimeArray, _super);
        function TimeArray() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this.id = [];
            _this.momentTime = []; // moment object
            _this.label = [];
            _this.unixTime = []; // unix time object
            _this.selections = [];
            _this.filter = [];
            _this.links = []; // all links at that time
            return _this;
        }
        return TimeArray;
    }(AttributeArray));
    networkcube.TimeArray = TimeArray;
    var LinkTypeArray = /** @class */ (function (_super) {
    ...
    var NodeTypeArray = /** @class */ (function (_super) {
    ...
    var LocationArray = /** @class */ (function (_super) {
